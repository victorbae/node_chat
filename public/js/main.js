...
var app = {
  ...
  chat: function(roomId, username) {
    ...
    socket.on('connect', function() {
      ...
      socket.on('addMessage', async function(message) {
        msgBufferCripto = message.content;
        console.log('MsgCript:  ' + message);
        console.log('roomId:  ' + $('#roomId').val());

        var enc = new TextDecoder("utf-8");
        var arr = new Uint8Array(msgBufferCripto);

        await app.helpers.decipher(enc.decode(arr), $('#roomId').val());
        //$('#msgCripto').val(message.content.toString());
      });
    });
  },
  ...
  helpers: {
    ...
    updateRoomsList: function(room) {
      ...
      if (room.isUsr == 'false') {
        if ($("#room ul li").length > 0) {
          $('#room ul').prepend(html);
        } else {
          $('#room ul').html('').html(html);
        }
      } else {
        if ($("#usr ul li").length > 0) {
          $('#usr ul').prepend(html);
        } else {
          $('#usr ul').html('').html(html);
        }
      }
    ...
    updateNumOfRooms: function() {
      var num = $('li.room-item').length;
      $('.room-num-rooms').text(num + " Sala(s)");
    },
...
$('.room-num-rooms').text($('li.room-item').length + " Sala(s)");
